...
train_dataset = MangaLandmarksDataset(Transforms(), folder=dataset_folder, split_set='train')
valid_dataset = MangaLandmarksDataset(TransformsNoAugmentation(), folder=dataset_folder, split_set='val')
...
selected_weight = 2
save_weight_name = ''
save_name = weights_ls[selected_weight].split('/')[-1]
for i, substring in enumerate(save_name.split("-")):
    if i != 2:
        save_weight_name += f'{substring}_'
model = BeitForImageClassification.from_pretrained(weights_ls[selected_weight])
model.classifier = nn.Linear(in_features=model.classifier.in_features, out_features=120, bias=True)
model.to(device)
...
experiment_name = f'{save_model_name}_{dataset_folder}_{attempt}'
